...
try:
    ...
    logger_init.error(f"An error occurred during mlx-audio or mlx.core import: {e}. Handler non-functional.",
        exc_info=True)
...
try:
    ...
    logger_init.warning("PyTorch (for Whisper) not found. Transcription of ref_audio will not be available in mlx_audio_handler.")  # noqa: E501
...
if TORCH_FOR_WHISPER_AVAILABLE:
    try:
        ...
        logger_init.warning("Torchaudio (for Whisper) not found. ref_audio loading/resampling for transcription might fail.")  # noqa: E501
    ...
    try:
        ...
        logger_init.warning("Transformers pipeline (for Whisper) not found. Transcription of ref_audio will not be available.")  # noqa: E501
...
try:
    ...
    logger_init.warning(f"Failed to import modules needed for mlx-audio Bark patch: {e_patch_imp}. Patch will not be applied if Bark is used.")  # noqa: E501
...
def _patched_load_voice_prompt(voice_prompt_input): #
    ...
    if isinstance(voice_prompt_input, str) and voice_prompt_input.endswith(".npz"): #
        if os.path.exists(voice_prompt_input): #
            ...
            try:
                return np.load(voice_prompt_input) #
            except Exception as e:
                raise ValueError(f"Could not load .npz: {voice_prompt_input}") from e #
        else:
            try:
                ...
                raise ValueError(f"Failed to load/download .npz: {voice_prompt_input}") from e #
    elif isinstance(voice_prompt_input, dict): #
        assert all(k in voice_prompt_input for k in ["semantic_prompt", "coarse_prompt", "fine_prompt"]) #  # noqa: S101
        ...
        if hasattr(bark_pipeline_module_for_patch, 'ALLOWED_PROMPTS') and \
           normalized_voice_name not in bark_pipeline_module_for_patch.ALLOWED_PROMPTS: #
            logger.warning(f"[Patched] Voice '{normalized_voice_name}' not in ALLOWED_PROMPTS. Using NPY load from '{BARK_VOICE_PROMPT_REPO_ID}'.") #  # noqa: E501
        ...
        if normalized_voice_name == "announcer":
            ...
            base_stem = normalized_voice_name #

        prompts_dict = {} #
        try:
            for key, suffix in {"semantic_prompt": "semantic_prompt.npy", "coarse_prompt": "coarse_prompt.npy",
                "fine_prompt": "fine_prompt.npy"}.items(): #
                filename = f"{base_stem}_{suffix}" if base_stem else suffix #
                path_in_repo = os.path.join(relative_dir, filename) #
                logger.info(f"[Patched] Downloading NPY: '{path_in_repo}' from '{BARK_VOICE_PROMPT_REPO_ID}' for '{key}'") #  # noqa: E501
            ...
            raise ValueError(f"Error processing NPY prompts for '{voice_prompt_input}': {e}") from e #
        ...
        try:
            if bark_pipeline_module_for_patch and hasattr(bark_pipeline_module_for_patch, '_load_voice_prompt'): #
                if bark_pipeline_module_for_patch._load_voice_prompt is _patched_load_voice_prompt: #
                    ...
                    return #
                logger.info(f"Applying patch to mlx_audio Bark voice loading. Prompts from: {BARK_VOICE_PROMPT_REPO_ID}") #  # noqa: E501
                ...
                logger.warning("[Patch] Could not apply Bark patch: _load_voice_prompt not found in mlx_audio.tts.models.bark.pipeline.") #  # noqa: E501
        except Exception as e:
            logger.error(f"[Patch] Error applying Bark patch: {e}", exc_info=True) #
...
def _detect_audio_format_pydub_inline(audio_path_str: str) -> str | None: #
    ...
    try:
        ...
        if any(fmt in format_name for fmt in ['mp4', 'm4a']):
            return 'mp4' #
        ...
        logger.error(f"mlx-audio: pydub mediainfo error for {audio_path_str}: {e}. Falling back to extension.",
            exc_info=False) #
        ext = Path(audio_path_str).suffix.lower().lstrip('.') #
        return ext if ext else None #
...
def _convert_audio_for_whisper_inline(audio_path_str: str, target_sr: int = 16000) -> tuple[Path | None, str | None]: #
    ...
    try:
        ...
        if PYDUB_AVAILABLE_FOR_TRIM and AudioSegment_pydub: #
            ...
            if not audio_format:
                raise ValueError(f"Could not determine audio format for '{audio_path_str}'.") #
            ...
            torchaudio_whisper_module.save(str(temp_wav_file_obj), waveform, sample_rate=target_sr,
                bits_per_sample=16) #
        else:
            if temp_wav_file_obj and temp_wav_file_obj.exists():
                temp_wav_file_obj.unlink(missing_ok=True) #
...
def _transcribe_ref_audio_for_mlx(
    ...
    if not all([TORCH_FOR_WHISPER_AVAILABLE, TORCHAUDIO_FOR_WHISPER_AVAILABLE,
        TRANSFORMERS_PIPELINE_FOR_WHISPER_AVAILABLE]): #
    ...
    logger.info(f"mlx-audio: Transcribing '{audio_path_str}' with Whisper '{whisper_model_id}'. Device: {target_device_str}, Lang: {language_hint}") #  # noqa: E501
    ...
    try:
        ...
        if language_hint:
            gen_kwargs["language"] = language_hint.lower() #
        ...
        if temp_wav_for_whisper and temp_wav_for_whisper.exists(): #
            try:
                temp_wav_for_whisper.unlink() #
            except OSError as e_del:
                logger.warning(f"mlx-audio: Could not delete temp Whisper WAV {temp_wav_for_whisper}: {e_del}") #
        del whisper_pipe #
        if TORCH_FOR_WHISPER_AVAILABLE and torch_whisper_module and target_device_str != "cpu": #
            ...
            elif hasattr(torch_whisper_module.backends,
                "mps") and torch_whisper_module.backends.mps.is_available() and hasattr(torch_whisper_module.mps,
                "empty_cache"): #
                try:
                    torch_whisper_module.mps.empty_cache() #
                except Exception:  # noqa: S110
                    pass #
        gc.collect() #
    return transcribed_text, err_msg #

def _trim_ref_audio_if_needed(ref_audio_path: Path, max_duration_ms: int,
    temp_dir_for_trimmed_audio: Path) -> tuple[Path, Path | None]: #
    ...
    try:
        ...
        if len(audio_segment) > max_duration_ms: #
            ...
            logger.info(f"mlx-audio: Trimmed ref audio from {len(audio_segment)/1000.0:.1f}s to {len(trimmed_segment)/1000.0:.1f}s. Using: {path_to_newly_trimmed_file}") #  # noqa: E501
...
def synthesize_with_mlx_audio(
    ...
    if not MLX_AUDIO_AVAILABLE or not generate_audio_mlx_func: #
        logger.error(f"mlx-audio ({crisptts_specific_model_id}): mlx-audio library or generate_audio function not available. Skipping.") #  # noqa: E501
        return #
    ...
    logger.info(f"mlx-audio ({crisptts_specific_model_id}): Cloning intent: {is_cloning_intent}. Voice/NPZ: '{voice_name_or_npz_path_for_mlx}'. Ref audio for cloning: '{actual_ref_audio_path_for_cloning}'.") #  # noqa: E501
    ...
    if pitch_val := crisptts_model_config.get("default_pitch"):
            ...
            for key, value in cli_params_json.items(): #
                ...
                if key in valid_keys: #
                    if key == "max_length_override": #
                        ...
                        logger.debug(f"mlx-audio ({crisptts_specific_model_id}): Set '{key}' to '{int(value)}' from model_params.") #  # noqa: E501
                    elif key in ["speed", "temperature", "top_p", "repetition_penalty", "streaming_interval", "pitch",
                        "cfg_scale"]: #
                        mlx_gen_kwargs[key] = float(value) #
                        logger.debug(f"mlx-audio ({crisptts_specific_model_id}): Overrode kwarg '{key}' to '{float(value)}'") #  # noqa: E501
                    elif key == "top_k": #
                        mlx_gen_kwargs[key] = int(value) #
                        logger.debug(f"mlx-audio ({crisptts_specific_model_id}): Overrode kwarg '{key}' to '{int(value)}'") #  # noqa: E501
                    elif key == "sentence_split_method" and isinstance(value, str) and value.lower() in ["none", "null",
                        "passthrough"]: #
                        ...
                        logger.debug(f"mlx-audio ({crisptts_specific_model_id}): Overrode kwarg '{key}' to '{str(value)}'") #  # noqa: E501
    ...
    if is_cloning_intent and actual_ref_audio_path_for_cloning: #
        if not custom_ref_text_from_params and crisptts_model_config.get("whisper_model_id_for_transcription"):
            ...
            if trimmed_for_transcription_path and trimmed_for_transcription_path.exists():
                ...
                logger.info(f"mlx-audio ({crisptts_specific_model_id}): Using trimmed audio '{audio_path_for_transcription}' for Whisper transcription.")  # noqa: E501
            else:
                logger.warning(f"mlx-audio ({crisptts_specific_model_id}): Trimming for transcription failed or not needed, using original '{actual_ref_audio_path_for_cloning}'. This might be too long.")  # noqa: E501
        ...
        if custom_ref_text_from_params: #
            ...
            logger.info(f"mlx-audio ({crisptts_specific_model_id}): Using ref_text from params: '{custom_ref_text_from_params[:100]}...'") #  # noqa: E501
        else: #
            whisper_model_id_cfg = crisptts_model_config.get("whisper_model_id_for_transcription") #
            if whisper_model_id_cfg: #
                logger.info(f"mlx-audio ({crisptts_specific_model_id}): No explicit ref_text. Transcribing ref_audio '{audio_path_for_transcription}' with local Whisper utility.") #  # noqa: E501
                whisper_target_device = "cpu" #
                if TORCH_FOR_WHISPER_AVAILABLE and torch_whisper_module: #
                    if torch_whisper_module.cuda.is_available():
                        whisper_target_device = "cuda" #
                    elif hasattr(torch_whisper_module.backends,
                        "mps") and torch_whisper_module.backends.mps.is_available(): #
                        if platform.machine() == "arm64" and platform.system() == "Darwin":
                            logger.info("mlx-audio: MLX on MPS, Whisper to CPU.") #
                        else:
                            whisper_target_device = "mps" #
                ...
                if trans_err or not transcribed_text: #
                    logger.error(f"mlx-audio ({crisptts_specific_model_id}): Transcription failed: {trans_err}. Using placeholder.") #  # noqa: E501
                    mlx_gen_kwargs["ref_text"] = " " #
                else: #
                    mlx_gen_kwargs["ref_text"] = transcribed_text #
            else: #
                logger.warning(f"mlx-audio ({crisptts_specific_model_id}): Cloning but Whisper not configured for this model. Dia might use internal STT or fail if ref_text is required.") #  # noqa: E501
    ...
    with tempfile.TemporaryDirectory(prefix="crisptts_mlx_handler_") as temp_dir_str: #
        ...
        if is_cloning_intent and actual_ref_audio_path_for_cloning:
            ...
            if "dia" in crisptts_specific_model_id.lower() and max_dur_ms > 12000: #
                logger.warning(f"mlx-audio ({crisptts_specific_model_id}): Reducing ref audio trim for Dia to 12s from {max_dur_ms/1000.0:.1f}s.") #  # noqa: E501
                max_dur_ms = 12000 #
        ...
        try:
            ...
            final_kwargs_for_generate = {k: v for k,
                v in mlx_gen_kwargs.items() if v is not None or k == "sentence_split_method"} #

            if is_cloning_intent: #
                if "voice" in final_kwargs_for_generate:
                    del final_kwargs_for_generate["voice"] #
            elif voice_name_or_npz_path_for_mlx: #
                 if "ref_audio" in final_kwargs_for_generate:
                     del final_kwargs_for_generate["ref_audio"] #
                 if "ref_text" in final_kwargs_for_generate:
                     del final_kwargs_for_generate["ref_text"] #
            # If neither, it's likely a model that doesn't need voice/ref_audio (e.g. Kokoro with lang_code)

            logger.debug(f"mlx-audio ({crisptts_specific_model_id}): Calling generate_audio with text: '{text[:50]}...', kwargs: {json.dumps(final_kwargs_for_generate, default=str)}") #  # noqa: E501
            ...
            if expected_temp_output_file.exists() and expected_temp_output_file.stat().st_size > 100: #
                ...
                logger.error(f"mlx-audio ({crisptts_specific_model_id}): Output file not created/empty: {expected_temp_output_file}") #  # noqa: E501
            ...
            # path_to_temp_trimmed_ref_for_generate_func is created within temp_dir_path, so it's auto-cleaned by TemporaryDirectory  # noqa: E501
    ...
    if temp_transcription_ref_dir:
        try:
            ...
            logger.debug(f"mlx-audio ({crisptts_specific_model_id}): Cleaned up temp directory for transcription ref: {temp_transcription_ref_dir.name}")  # noqa: E501
        except Exception as e_cleanup:
            logger.warning(f"mlx-audio ({crisptts_specific_model_id}): Error cleaning up temp transcription ref dir {temp_transcription_ref_dir.name}: {e_cleanup}")  # noqa: E501
    ...
    if generated_audio_data_bytes: #
        if output_file_str: #
            ...
            try:
                save_audio(generated_audio_data_bytes, str(final_path), False, f"{out_fmt}_bytes",
                    crisptts_model_config.get("sample_rate")) #
            except Exception as e_save:
                logger.error(f"mlx-audio ({crisptts_specific_model_id}): Error saving: {e_save}", exc_info=True) #
        if play_direct: #
            play_audio(generated_audio_data_bytes, False, f"{mlx_gen_kwargs.get('audio_format', 'wav')}_bytes",
                crisptts_model_config.get("sample_rate")) #
    else: #
        logger.warning(f"mlx-audio ({crisptts_specific_model_id}): No audio bytes generated for '{mlx_model_repo_id_or_path}'.") #  # noqa: E501
    ...
    if MLX_AUDIO_AVAILABLE and mx_core_module and hasattr(mx_core_module, 'clear_cache'): #
        try:
            ...
            logger.warning(f"mlx-audio: Error clearing MLX cache: {e}") #
    gc.collect() #
