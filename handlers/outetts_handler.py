...
if TORCH_AVAILABLE_IN_HANDLER:
    try:
        ...
        OuteTTSInterface_h, OuteTTSModelConfig_h, OuteTTSModels_h, OuteTTSBackend_h = _Interface, _ModelConfig, _Models, _Backend  # noqa: E501
        OuteTTSLlamaCppQuantization_h, OuteTTSGenerationConfig_h = _LlamaCppQuantization, _GenerationConfig
        OuteTTSGenerationType_h, OuteTTSSamplerConfig_h, OuteTTSInterfaceVersion_h = _GenerationType, _SamplerConfig, _InterfaceVersion  # noqa: E501
        ...
        logger_init.warning(f"OuteTTS library import failed (AttributeError: {e_attr}). Check components/protobuf. Non-functional.")  # noqa: E501
...
from config import OUTETTS_VERSION_STRING_MODEL_CONFIG_DATA  # noqa: E402
from utils import SuppressOutput, _prepare_oute_speaker_ref  # noqa: E402

logger = logging.getLogger("CrispTTS.handlers.outetts")

def synthesize_with_outetts_local(model_config, text, voice_id_or_path_override, model_params_override, output_file_str,
    play_direct):
    ...
    if isinstance(backend_choice_from_config, OuteTTSBackend_h):
        ...
        if "LLAMACPP" in backend_choice_from_config.upper():
            actual_backend_enum = OuteTTSBackend_h.LLAMACPP
        elif "HF" in backend_choice_from_config.upper():
            actual_backend_enum = OuteTTSBackend_h.HF
        else:
            logger.error(f"OuteTTS ({crisptts_model_id}) - Unknown backend string: '{backend_choice_from_config}'.")
            return
    else:
        logger.error(f"OuteTTS ({crisptts_model_id}) - Invalid 'backend_to_use' type: {type(backend_choice_from_config)}.")  # noqa: E501
        return
    ...
    if OuteTTSModels_h:
        if isinstance(outetts_model_enum_from_config, OuteTTSModels_h):
            ...
            try:
                actual_model_enum = getattr(OuteTTSModels_h, outetts_model_enum_from_config.replace("_STR_FALLBACK",
                    ""))
            except AttributeError:
                logger.warning(f"OuteTTS ({crisptts_model_id}) - Could not map model string '{outetts_model_enum_from_config}' to enum.")  # noqa: E501
        elif outetts_model_enum_from_config is None:
            logger.info(f"OuteTTS ({crisptts_model_id}) - 'outetts_model_enum' is None in config (expected for HF).")
        else:
            logger.error(f"OuteTTS ({crisptts_model_id}) - Unexpected 'outetts_model_enum' type: {type(outetts_model_enum_from_config)}.")  # noqa: E501
            return
    else:
        logger.warning(f"OuteTTS ({crisptts_model_id}) - OuteTTS.Models enum class not available.")

    logger.info(f"OuteTTS ({crisptts_model_id}) - Resolved Backend: {actual_backend_enum}, Derived Model Enum: {actual_model_enum}")  # noqa: E501
    ...
    try:
        ...
        if actual_backend_enum == OuteTTSBackend_h.LLAMACPP:
            if not actual_model_enum:
                logger.error(f"OuteTTS ({crisptts_model_id}) - LlamaCPP: 'outetts_model_enum' is required.")
                return
            model_config_constructor_params["model"] = actual_model_enum
            if not OuteTTSLlamaCppQuantization_h:
                logger.error(f"OuteTTS ({crisptts_model_id}) - LlamaCppQuantization enum NA.")
                return
            quant_cfg = model_config.get("quantization_to_use")
            if isinstance(quant_cfg, OuteTTSLlamaCppQuantization_h):
                model_config_constructor_params["quantization"] = quant_cfg
            elif isinstance(quant_cfg, str):
                model_config_constructor_params["quantization"] = getattr(OuteTTSLlamaCppQuantization_h,
                    quant_cfg.replace("_STR_FALLBACK", ""))
            else:
                model_config_constructor_params["quantization"] = OuteTTSLlamaCppQuantization_h.FP16
            logger.debug(f"OuteTTS ({crisptts_model_id}) - LlamaCPP auto_config params: {model_config_constructor_params}")  # noqa: E501
            ...
            if actual_model_enum and hasattr(actual_model_enum, 'value'):
                ...
                if tokenizer_path and isinstance(tokenizer_path, str) and "/" in tokenizer_path:
                    ...
                    logger.error(f"OuteTTS ({crisptts_model_id}) - HF: Cannot determine HF model ID.")
                    return
            ...
            iface_ver_val = model_config.get("interface_version_enum",
                crisptts_ver_cfg.get("interface_version_enum_val", "V3_STR_FALLBACK"))
            actual_iface_ver = OuteTTSInterfaceVersion_h.V3
            if isinstance(iface_ver_val, OuteTTSInterfaceVersion_h):
                actual_iface_ver = iface_ver_val
            elif isinstance(iface_ver_val, str):
                try:
                    actual_iface_ver = getattr(OuteTTSInterfaceVersion_h, iface_ver_val.replace("_STR_FALLBACK", ""))
                except AttributeError:
                    logger.warning(f"OuteTTS ({crisptts_model_id}) - Invalid HF interface version string '{iface_ver_val}'. Defaulting to V3.")  # noqa: E501
            model_config_constructor_params["interface_version"] = actual_iface_ver

            if IS_MPS_HANDLER and TORCH_AVAILABLE_IN_HANDLER:
                logger.info(f"OuteTTS ({crisptts_model_id}) - Configuring HF backend for MPS. Model ID: {full_hf_model_id}")  # noqa: E501
                # Prioritize float16 for MPS to reduce memory, respecting config if it specifically asks for float32
                dtype_from_config_str = model_config.get("torch_dtype_for_hf_wrapper",
                    "torch.float16_STR_FALLBACK") # Default to float16 string
                ...
                logger.debug(f"OuteTTS ({crisptts_model_id}) - HF non-MPS auto_config using params: {model_config_constructor_params}")  # noqa: E501
                interface_config_obj = OuteTTSModelConfig_h.auto_config(**model_config_constructor_params)
        else:
            logger.error(f"OuteTTS ({crisptts_model_id}) - Unhandled backend: {actual_backend_enum}")
            return

        if not interface_config_obj:
            logger.error(f"OuteTTS ({crisptts_model_id}) - Failed to create ModelConfig.")
            return
        logger.info(f"OuteTTS ({crisptts_model_id}) - ModelConfig: Device='{interface_config_obj.device}', Dtype='{interface_config_obj.dtype}'")  # noqa: E501
        ...
        if is_internal_id:
            ...
            try:
                ...
                logger.error(f"OuteTTS ({crisptts_model_id}) - Failed to load speaker ID '{speaker_input_arg}': {e}")
                return
        else:
            ...
            if not processed_wav:
                ...
                if temp_speaker_file_to_delete:
                    Path(temp_speaker_file_to_delete).unlink(missing_ok=True)
                return
            logger.info(f"OuteTTS ({crisptts_model_id}) - Creating speaker profile from: {processed_wav} (suppressing verbose lib output)...")  # noqa: E501
            try:
                with SuppressOutput(suppress_stdout=not logger.isEnabledFor(logging.DEBUG), suppress_stderr=True):
        ...
        max_len = interface_config_obj.max_sequence_length if hasattr(interface_config_obj,
            'max_sequence_length') else 8192
        if model_params_override:
            try:
                cli_params = json.loads(model_params_override)
                for k, v_param in cli_params.items():
                    if hasattr(OuteTTSSamplerConfig_h(), k):
                        sampler_vals[k] = v_param
                    elif k == "max_length":
                        max_len = int(v_param)
            except (json.JSONDecodeError, ValueError) as e:
                logger.warning(f"OuteTTS ({crisptts_model_id}) - Error parsing --model-params: {e}")
        ...
        gen_cfg_obj = OuteTTSGenerationConfig_h(text=text, generation_type=OuteTTSGenerationType_h.CHUNKED,
                                             speaker=speaker_profile_obj, sampler_config=sampler_obj, max_length=max_len)  # noqa: E501
        ...
        if actual_backend_enum == OuteTTSBackend_h.HF and platform.system() == "Darwin" and \
           any(m in str(e_synth).lower() for m in ["no such file", "can't load tokenizer", "can't load model",
               "mpsgraph"]):
            logger.warning(f"OuteTTS ({crisptts_model_id}) - macOS HF/MPS error hint: Clear mps_graph_compiler cache and/or Hugging Face cache.")  # noqa: E501
    finally:
        ...
        if TORCH_AVAILABLE_IN_HANDLER:
            if IS_MPS_HANDLER and hasattr(torch_oute, "mps") and hasattr(torch_oute.mps, "empty_cache"):
                try:
                    torch_oute.mps.empty_cache()
                    logger.debug(f"OuteTTS ({crisptts_model_id}) - MPS cache cleared.")
                except Exception as e:
                    logger.warning(f"OuteTTS ({crisptts_model_id}) - Failed to clear MPS cache: {e}")
            elif torch_oute.cuda.is_available() and hasattr(torch_oute.cuda, "empty_cache"):
                torch_oute.cuda.empty_cache()
                logger.debug(f"OuteTTS ({crisptts_model_id}) - CUDA cache cleared.")
        if temp_speaker_file_to_delete:
            try:
                Path(temp_speaker_file_to_delete).unlink(missing_ok=True)
            except OSError as e:
                logger.warning(f"OuteTTS ({crisptts_model_id}) - Failed to delete temp WAV {temp_speaker_file_to_delete}: {e}")  # noqa: E501
