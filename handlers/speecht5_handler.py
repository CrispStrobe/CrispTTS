...
def synthesize_with_speecht5_transformers(model_config, text, voice_id_override, model_params_override, output_file_str,
    play_direct):
    ...
    if not all([model_id, vocoder_id, speaker_embeddings_repo, default_speaker_idx is not None]):
        logger.error("SpeechT5: Incomplete model configuration (model_id, vocoder_id, speaker_embeddings_repo, or default_speaker_embedding_index missing).")  # noqa: E501
        return
    ...
    try:
        ...
        if voice_id_override:
            if Path(voice_id_override).is_file() and voice_id_override.lower().endswith((".pt", ".pth")):
                try:
                    ...
                    logger.warning(f"SpeechT5 - Failed to load custom speaker embedding '{voice_id_override}': {e}. Trying default/index.")  # noqa: E501
            if not speaker_embedding_to_use and voice_id_override.isdigit():
                try:
                    speaker_idx = int(voice_id_override)
                    embeddings_dataset = load_dataset_s5(speaker_embeddings_repo, split="validation",
                        trust_remote_code=True)
                    speaker_embedding_to_use = torch_s5.tensor(embeddings_dataset[speaker_idx]["xvector"]).unsqueeze(0)
                    logger.info(f"SpeechT5 - Using speaker embedding index {speaker_idx} from {speaker_embeddings_repo}.")  # noqa: E501
                except Exception as e:
                    logger.warning(f"SpeechT5 - Failed to load embedding index {voice_id_override}: {e}. Using default.")  # noqa: E501

        if speaker_embedding_to_use is None:
            try:
                embeddings_dataset = load_dataset_s5(speaker_embeddings_repo, split="validation",
                    trust_remote_code=True)
                speaker_embedding_to_use = torch_s5.tensor(embeddings_dataset[default_speaker_idx]["xvector"]).unsqueeze(0)  # noqa: E501
                logger.info(f"SpeechT5 - Using default speaker index {default_speaker_idx}.")
            except Exception as e:
                logger.error(f"SpeechT5 - Failed to load default speaker embedding: {e}")
                return

        device = "cuda" if torch_s5.cuda.is_available() else ("mps" if (hasattr(torch_s5.backends,
            "mps") and torch_s5.backends.mps.is_available()) else "cpu")
        ...
        del processor, model, vocoder, speaker_embedding_to_use, inputs
        gc.collect()
        if TORCH_AVAILABLE_IN_HANDLER and torch_s5.cuda.is_available():
            torch_s5.cuda.empty_cache()
