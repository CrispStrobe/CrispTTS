...
# CrispTTS Utils
from utils import SuppressOutput, play_audio  # noqa: E402
...
try:
    ...
    logger_init.warning("PyTorch or Torchaudio not found. LLaSA Multilingual Transformers handler will be non-functional.")  # noqa: E501
...
if TORCH_AVAILABLE: # Only attempt these if torch is available
    try:
        ...
        logger_init.warning("Transformers library (or its `pipeline` component) not found. Transcription will be unavailable.")  # noqa: E501
    ...
    try:
        ...
        logger_init.warning("XCodec2 library not found. LLaSA Multilingual Transformers handler will be non-functional.")  # noqa: E501
...
def synthesize_with_llasa_multilingual_transformers(
    ...
    device_str = "cpu"
    if IS_CUDA_LLASA_MULTI:
        device_str = "cuda"
    elif IS_MPS_LLASA_MULTI:
        device_str = "mps"
    ...
    try:
        ...
        llasa_tokenizer_inst = AutoTokenizer_llasa_multi.from_pretrained(tokenizer_id, token=hf_token,
            trust_remote_code=True)

        logger.info(f"{log_prefix}Loading LLaSA LLM from '{llm_model_id}' to device '{device_str}'...")
        llasa_llm_inst = AutoModelForCausalLM_llasa_multi.from_pretrained(llm_model_id, token=hf_token,
            trust_remote_code=True)
        ...
        if actual_ref_audio_path_str and isinstance(actual_ref_audio_path_str,
            str) and actual_ref_audio_path_str.strip().lower() != 'none':
            ...
            if resolved_ref_path.exists() and resolved_ref_path.is_file():
                ...
                if sr_ref != target_ref_sr_for_xcodec:
                    resampler = torchaudio_llasa_multi.transforms.Resample(orig_freq=sr_ref,
                        new_freq=target_ref_sr_for_xcodec)
                ...
                if model_params_override:
                    try:
                        custom_ref_text = json.loads(model_params_override).get("reference_text")
                ...
                if custom_ref_text and isinstance(custom_ref_text, str) and custom_ref_text.strip():
                    ...
                    # --- ADDED DEBUG LOGS ---
                    logger.debug(f"{log_prefix}Whisper Check: whisper_model_id_cfg = '{whisper_model_id_cfg}' (type: {type(whisper_model_id_cfg)})")  # noqa: E501
                    logger.debug(f"{log_prefix}Whisper Check: pipeline_llasa_multi is {'NOT None and callable' if callable(pipeline_llasa_multi) else ('None or not callable')} (type: {type(pipeline_llasa_multi)})")  # noqa: E501
                    ...
                    if (whisper_model_id_cfg
                                                        and pipeline_llasa_multi and callable(pipeline_llasa_multi)):
                        logger.info(f"{log_prefix}Preparing to transcribe ref audio '{resolved_ref_path}' with Whisper: '{whisper_model_id_cfg}'")  # noqa: E501
                        ...
                        if prep_error or not temp_whisper_audio_file_to_delete:
                            logger.error(f"{log_prefix}Failed to prepare audio for Whisper: {prep_error}. Using placeholder transcription.")  # noqa: E501
                            ref_transcription = "Reference audio preparation failed for Whisper."
                        else:
                            logger.debug(f"{log_prefix}Ref audio prepared for Whisper at temp path: {temp_whisper_audio_file_to_delete}")  # noqa: E501
                            whisper_pipeline_device_arg = device_str
                            if device_str == "cuda":
                                whisper_pipeline_device_arg = 0

                            cache_key = (whisper_model_id_cfg, whisper_pipeline_device_arg,
                                str(torch_llasa_multi.float16 if device_str != "cpu" else torch_llasa_multi.float32))
                            if cache_key in _llasa_multilingual_whisper_pipeline_cache:
                                ...
                                whisper_pipeline_inst = pipeline_llasa_multi(
                                    ...
                                    torch_dtype=torch_llasa_multi.float16 if device_str != "cpu" else torch_llasa_multi.float32,  # noqa: E501
                            ...
                            whisper_gen_kwargs = {"language": lang_hint_for_whisper.lower()} if lang_hint_for_whisper else {}  # noqa: E501
                            whisper_gen_kwargs["return_timestamps"] = True

                            logger.debug(f"{log_prefix}Calling Whisper ASR pipeline (model: {whisper_model_id_cfg}) with generate_kwargs: {whisper_gen_kwargs}")  # noqa: E501
                            with SuppressOutput(suppress_stderr=True,
                                suppress_stdout=not logger.isEnabledFor(logging.DEBUG)):
                                transcription_output = whisper_pipeline_inst(str(temp_whisper_audio_file_to_delete),
                                    generate_kwargs=whisper_gen_kwargs)

                            transcribed_text_str = transcription_output.get("text", "").strip()
                            if not transcribed_text_str and "chunks" in transcription_output:
                                transcribed_text_str = " ".join([chunk.get('text',
                                    "").strip() for chunk in transcription_output["chunks"]]).strip()
                        ...
                        logger.warning(f"{log_prefix}No custom reference text AND (Whisper model not configured OR Transformers pipeline not available). Using placeholder transcription.")  # noqa: E501
                ...
                with torch_llasa_multi.no_grad():
                    vq_codes_from_ref_tensor = codec_model_inst.encode_code(input_waveform=processed_ref_waveform_for_trimming.to(device_str))  # noqa: E501
                ...
                logger.info(f"{log_prefix}Generated {len(speech_ids_prefix_str_list)} speech prefix tokens from reference audio.")  # noqa: E501
            else:
                is_cloning_mode = False
                input_text_for_llm = text
                if "clone" in crisptts_model_id_for_log.lower():
                    logger.error(f"{log_prefix}Cloning intended, but ref audio '{resolved_ref_path}' not valid. Aborting.")  # noqa: E501
                    return
        ...
        logger.debug(f"{log_prefix}Chat prompt for LLM (assistant starts with '{assistant_start_content[:60]}...'): {json.dumps(chat_prompt_list, ensure_ascii=False, indent=2)[:500]}...")  # noqa: E501
        ...
        if model_params_override:
            try:
                ...
                for k_param, v_param in cli_gen_params.items():
                    if k_param in valid_hf_keys:
                        generation_params[k_param] = v_param
            except (json.JSONDecodeError, TypeError):
                pass # Already logged warning if needed
        ...
        with torch_llasa_multi.no_grad(), SuppressOutput(suppress_stdout=not logger.isEnabledFor(logging.DEBUG),
        ...
        if (generated_ids_after_prompt.numel() > 0
                and generated_ids_after_prompt[-1] == generation_params["eos_token_id"]):
            generated_ids_after_prompt = generated_ids_after_prompt[:-1]

        generated_token_strings_from_llm = llasa_tokenizer_inst.convert_ids_to_tokens(generated_ids_after_prompt.tolist())  # noqa: E501
        ...
        if not final_numeric_speech_ids_for_xcodec:
            logger.error(f"{log_prefix}No valid numeric speech IDs extracted. Cannot decode.")
            return
        logger.info(f"{log_prefix}Extracted {len(final_numeric_speech_ids_for_xcodec)} numeric speech IDs for XCodec2.")

        xcodec_input_tensor = torch_llasa_multi.tensor(final_numeric_speech_ids_for_xcodec,
            device=device_str).unsqueeze(0).unsqueeze(0)
        with torch_llasa_multi.no_grad():
        ...
        if generated_waveform_decoded is None or generated_waveform_decoded.numel() == 0:
            logger.error(f"{log_prefix}XCodec2 decoding returned None or empty tensor.")
            return
        ...
        if is_cloning_mode and processed_ref_waveform_for_trimming is not None:
            ...
            if model_params_override:
                try:
                    trim_reconstructed_audio = json.loads(model_params_override).get("trim_reconstructed_prompt", True)
                except (json.JSONDecodeError, TypeError):
                    pass
        ...
        if audio_output_np_final.size == 0:
            logger.error(f"{log_prefix}Final audio is empty after XCodec2 and potential trimming.")
            return
        ...
        if play_direct:
            try:
                audio_int16_bytes = (numpy_llasa_multi.clip(audio_output_np_final, -1.0,
                    1.0) * 32767).astype(numpy_llasa_multi.int16).tobytes()
        ...
        if temp_whisper_audio_file_to_delete and temp_whisper_audio_file_to_delete.exists():
            try:
                ...
                logger.debug(f"{log_prefix}Deleted temp Whisper audio: {temp_whisper_audio_file_to_delete}")
                 ...
                 logger.warning(f"{log_prefix}Could not delete temp Whisper audio '{temp_whisper_audio_file_to_delete}': {e_del_temp}")  # noqa: E501

        if TORCH_AVAILABLE and torch_llasa_multi:
            if device_str == "cuda":
                torch_llasa_multi.cuda.empty_cache()
            elif device_str == "mps" and hasattr(torch_llasa_multi.mps, "empty_cache"):
                try:
                    torch_llasa_multi.mps.empty_cache()
