...
try:
    ...
    logger_init.warning(f"MLX/mlx-lm or critical components not found. LLaSA Hybrid handler will be non-functional. Error: {e}")  # noqa: E501
...
try:
    ...
    logger_init.info(f"PyTorch imported. Torchaudio for resampling: {'Available' if TORCHAUDIO_AVAILABLE_FOR_LLASA else 'Unavailable'}.")  # noqa: E501
except ImportError:
    logger_init.warning("PyTorch or torchaudio not found. LLaSA XCodec2 or resampling fallback will be non-functional.")
    if 'torch_llasa' in locals() and torch_llasa is None:
        TORCH_FOR_LLASA_AVAILABLE = False # type: ignore
    TORCHAUDIO_AVAILABLE_FOR_LLASA = False
except Exception as e:
...
try:
    ...
    logger_init.info("pydub not available for LLaSA handler. Reference audio trimming will use numpy slicing if pydub is selected but unavailable.")  # noqa: E501
...
def _validate_and_prepare_reference_audio(
    ...
    logger.debug(f"LLaSA Prep Ref Audio: Input path: '{ref_wav_path_str}', Target SR: {target_sr}, Max Duration: {max_duration_s}s")  # noqa: E501
    if not SF_FOR_LLASA_AVAILABLE or not sf_llasa:
        logger.error("LLaSA Prep Ref Audio: SoundFile library (sf_llasa) is not available. Cannot process reference audio.")  # noqa: E501
        return None
    ...
    if not ref_wav_path_actual.is_absolute():
        ...
        logger.debug(f"LLaSA Prep Ref Audio: Relative path '{ref_wav_path_str}' resolved to '{resolved_path}' against project root '{project_root}'.")  # noqa: E501
    ...
    try:
        ...
        logger.debug(f"LLaSA Prep Ref Audio: Loaded audio shape: {prompt_wav_samples.shape}, Original SR: {sr_prompt}Hz")  # noqa: E501
        ...
        if prompt_wav_samples.ndim > 1:
            ...
            logger.info(f"LLaSA Prep Ref Audio: Converted multi-channel audio from '{ref_wav_path_actual}' to mono. New shape: {prompt_wav_samples.shape}")  # noqa: E501

        current_duration_s = len(prompt_wav_samples) / sr_prompt
        logger.debug(f"LLaSA Prep Ref Audio: Duration after mono conversion (if any): {current_duration_s:.2f}s at {sr_prompt}Hz.")  # noqa: E501

        if current_duration_s > max_duration_s:
            logger.info(f"LLaSA Prep Ref Audio: Reference audio ({current_duration_s:.1f}s) > {max_duration_s}s. Attempting to trim.")  # noqa: E501
            if PYDUB_LLASA_AVAILABLE and AudioSegment_llasa_pydub:
                try:
                    ...
                    with tempfile.NamedTemporaryFile(suffix=ref_wav_path_actual.suffix or ".wav",
                        prefix="llasa_trimmed_pydub_", delete=False) as tmp_f:
                        temp_trimmed_file_to_delete = Path(tmp_f.name)
                    trimmed_segment.export(str(temp_trimmed_file_to_delete),
                        format=ref_wav_path_actual.suffix.lstrip('.') or 'wav')
                    prompt_wav_samples, sr_prompt = sf_llasa.read(str(temp_trimmed_file_to_delete), dtype='float32',
                        always_2d=False)
                    if prompt_wav_samples.ndim > 1:
                        prompt_wav_samples = np.mean(prompt_wav_samples, axis=1)
                    current_duration_s = len(prompt_wav_samples) / sr_prompt
                    logger.info(f"LLaSA Prep Ref Audio: Successfully trimmed with pydub to {current_duration_s:.1f}s. Temp file: {temp_trimmed_file_to_delete}")  # noqa: E501
                except Exception as e_pydub_trim:
                    logger.warning(f"LLaSA Prep Ref Audio: pydub trimming failed ('{e_pydub_trim}'). Falling back to numpy slice.", exc_info=logger.isEnabledFor(logging.DEBUG))  # noqa: E501
                    ...
                    if len(prompt_wav_samples) > num_samples_to_keep:
                        ...
                        current_duration_s = len(prompt_wav_samples) / sr_prompt
                        logger.info(f"LLaSA Prep Ref Audio: Trimmed using numpy slice to {current_duration_s:.1f}s (after pydub fail).")  # noqa: E501
            else:
            ...
            logger.debug(f"LLaSA Prep Ref Audio: No trimming needed (duration {current_duration_s:.1f}s <= {max_duration_s}s).")  # noqa: E501
        ...
        if sr_prompt != target_sr:
            ...
            if SCIPY_AVAILABLE_FOR_LLASA and scipy_resample_func:
                ...
                if num_target_samples > 0:
                    ...
                    sr_prompt = target_sr
                    logger.debug(f"LLaSA Prep Ref Audio: Resampled with Scipy. New shape: {prompt_wav_samples.shape}, new SR: {sr_prompt}Hz.")  # noqa: E501
                else:
                    logger.warning(f"LLaSA Prep Ref Audio: Scipy resampling would result in 0 samples. Using original SR {sr_prompt}Hz.")  # noqa: E501
                ...
                try:
                    ...
                    logger.error(f"LLaSA Prep Ref Audio: torchaudio resampling failed: {e_torchaudio_resample}. Using original SR {sr_prompt}Hz.", exc_info=True)  # noqa: E501
            else:
                logger.warning(f"LLaSA Prep Ref Audio: Neither Scipy nor torchaudio available for resampling. Using original SR {sr_prompt}Hz. XCodec2 expects {target_sr}Hz; quality may be impacted.")  # noqa: E501
        else:
        ...
        if final_duration_s < 0.5:
            logger.warning(f"LLaSA Prep Ref Audio: Final reference audio is very short ({final_duration_s:.1f}s). Cloning quality may be poor.")  # noqa: E501

        return str(ref_wav_path_actual), prompt_wav_samples, sr_prompt, temp_trimmed_file_to_delete

    except Exception as e_audio_proc:
        logger.error(f"LLaSA Prep Ref Audio: Critical error during loading/processing of reference audio '{ref_wav_path_actual}': {e_audio_proc}", exc_info=True)  # noqa: E501
        if temp_trimmed_file_to_delete and temp_trimmed_file_to_delete.exists():
            try:
                temp_trimmed_file_to_delete.unlink(missing_ok=True)
            except OSError:
...
def _validate_dependencies() -> Tuple[bool, List[str]]:
    ...
    if not TRANSFORMERS_FOR_LLASA_AVAILABLE:
        missing_deps.append("Transformers (for LLaSA chat)")
    if not TORCH_FOR_LLASA_AVAILABLE:
        missing_deps.append("PyTorch (for LLaSA XCodec2)")
    if not SF_FOR_LLASA_AVAILABLE:
        missing_deps.append("SoundFile")
    if not UTILS_AVAILABLE:
        missing_deps.append("CrispTTS Project Utils")
    return len(missing_deps) == 0, missing_deps

def _validate_model_objects() -> Tuple[bool, List[str]]:
    missing_objects = []
    if not mlx_lm_load:
        missing_objects.append("mlx_lm.load function")
    if not mlx_lm_generate_str_func:
        missing_objects.append("mlx_lm.generate.generate function")
    if not make_sampler_func:
        missing_objects.append("mlx_lm.sample_utils.make_sampler function")
    if not XCodec2Model_llasa:
        missing_objects.append("XCodec2Model class")
    if not AutoTokenizer_llasa_chat:
        missing_objects.append("AutoTokenizer class (for LLaSA chat)")
    if not torch_llasa:
        missing_objects.append("torch module (for LLaSA)")
    if not sf_llasa:
        missing_objects.append("soundfile module (for LLaSA)")
...
def _llasa_extract_speech_ids_from_str_list(speech_tokens_text: str) -> List[int]:
    ...
    for num_str in matches:
        try:
            ...
            logger.warning(f"LLaSA Extract Speech IDs: Could not parse int from '{num_str}' in '{speech_tokens_text[:100]}...'")  # noqa: E501

    logger.debug(f"LLaSA Extract Speech IDs: Input (first 100): '{speech_tokens_text[:100]}...', Extracted IDs: {len(speech_ids)} (e.g., {speech_ids[:10]})")  # noqa: E501
    return speech_ids
...
def _get_pytorch_device() -> Tuple[str, Optional['torch_llasa.device']]: # type: ignore
    ...
    try:
        return device_str, torch_llasa.device(device_str)
    except Exception as e:
...
def _cleanup_resources(*resources_to_del: object) -> None:
    ...
    if TORCH_FOR_LLASA_AVAILABLE and torch_llasa:
        try:
            ...
            if TORCHAUDIO_AVAILABLE_FOR_LLASA and hasattr(torch_llasa.backends,
                "mps") and torch_llasa.backends.mps.is_available() and hasattr(torch_llasa.mps, "empty_cache"):
...
def synthesize_with_llasa_hybrid(
    ...
    logger.debug(f"{log_prefix}Voice ID/Path override from main: '{voice_id_override}' (type: {type(voice_id_override)})")  # noqa: E501
    ...
    if not objects_valid:
        logger.error(f"{log_prefix}Essential Python objects not loaded: {', '.join(missing_objects)}. Skipping synthesis.")  # noqa: E501
        return
    ...
    logger.debug(f"{log_prefix}Core Config: LLM='{llm_model_id_cfg}', ChatTokenizer='{chat_tokenizer_id_cfg}', Codec='{codec_model_id_cfg}', TargetSR={target_sample_rate}Hz")  # noqa: E501
    if not all([llm_model_id_cfg, chat_tokenizer_id_cfg, codec_model_id_cfg]):
        logger.error(f"{log_prefix}Missing critical model IDs (LLM, ChatTokenizer, or Codec) in configuration. Skipping synthesis.")  # noqa: E501
        return
    ...
    if _is_fallback_attempt:
        ...
        logger.debug(f"{log_prefix}Initial candidate_ref_path: '{candidate_ref_path}' (from voice_id_override or config default_voice_id)")  # noqa: E501
    ...
    logger.info(f"{log_prefix}Effective reference audio path for processing: '{actual_ref_speaker_wav_for_processing}' (type: {type(actual_ref_speaker_wav_for_processing)})")  # noqa: E501
    ...
    llasa_llm_mlx_model, llasa_llm_mlx_tokenizer, chat_template_hf_tokenizer, codec_model_pt_inst = None, None, None, None  # noqa: E501
    ...
    try:
        ...
        with SuppressOutput_util(suppress_stdout=suppress_external_lib_logs,
            suppress_stderr=suppress_external_lib_logs):
            try:
            ...
            try:
                ...
                logger.error(f"{log_prefix}Failed to load HF Chat Template Tokenizer '{chat_tokenizer_id_cfg}': {e_hf_tok_load}", exc_info=True)  # noqa: E501
                return

            try:
                logger.debug(f"{log_prefix}Loading PyTorch XCodec2 Model: '{codec_model_id_cfg}' to device '{pt_device_name_str}'")  # noqa: E501
                ...
                logger.error(f"{log_prefix}Failed to load PyTorch XCodec2 Model '{codec_model_id_cfg}': {e_xcodec_load}", exc_info=True)  # noqa: E501
                return
        ...
        if actual_ref_speaker_wav_for_processing:
            logger.info(f"{log_prefix}Attempting voice cloning using reference: '{actual_ref_speaker_wav_for_processing}'")  # noqa: E501
            ...
            if ref_audio_validation_result:
                path_str_from_validation, audio_samples_np, actual_sr, temp_file_to_del_from_validation = ref_audio_validation_result  # noqa: E501
                ...
                if actual_sr == target_sample_rate:
                    logger.info(f"{log_prefix}Encoding reference audio ('{path_str_from_validation}', {len(audio_samples_np)/actual_sr:.1f}s @ {actual_sr}Hz) with XCodec2...")  # noqa: E501
                    try:
                        with torch_llasa.no_grad():
                            ref_audio_tensor = torch_llasa.from_numpy(audio_samples_np).float().unsqueeze(0).to(pt_device_obj)  # noqa: E501
                        ...
                        if vq_codes_from_ref:
                            ...
                            logger.info(f"{log_prefix}Generated {len(vq_codes_from_ref)} speech prefix tokens from reference audio for cloning.")  # noqa: E501
                            ...
                            logger.warning(f"{log_prefix}XCodec2 encoding of reference audio yielded no VQ codes. Using basic prefix.")  # noqa: E501
                            assistant_content_prefix_str = "<|SPEECH_GENERATION_START|>"
                            is_cloning_mode = False
                    except Exception as e_encode:
                        logger.error(f"{log_prefix}Failed to encode reference audio with XCodec2: {e_encode}",
                            exc_info=True)
                        assistant_content_prefix_str = "<|SPEECH_GENERATION_START|>"
                        is_cloning_mode = False
                else:
                    logger.warning(f"{log_prefix}Reference audio SR after validation ({actual_sr}Hz) does not match target SR ({target_sample_rate}Hz). This is unexpected. Skipping VQ encoding for cloning.")  # noqa: E501
                    assistant_content_prefix_str = "<|SPEECH_GENERATION_START|>"
                    is_cloning_mode = False
            else:
                logger.warning(f"{log_prefix}Validation/preparation of reference audio failed. No VQ codes for cloning prefix. Proceeding with basic prefix.")  # noqa: E501
                assistant_content_prefix_str = "<|SPEECH_GENERATION_START|>"
                is_cloning_mode = False
        else:
            logger.info(f"{log_prefix}Operating in zero-shot mode (no valid reference audio path). Using basic speech generation prefix.")  # noqa: E501
            assistant_content_prefix_str = "<|SPEECH_GENERATION_START|>"
            is_cloning_mode = False
        ...
        user_content = f"Convert the text to speech:<|TEXT_UNDERSTANDING_START|>{combined_text}<|TEXT_UNDERSTANDING_END|>"  # noqa: E501
        chat_for_template = [
        ...
        try:
            logger.debug(f"{log_prefix}Applying chat template. User content (text part): '{text_to_synthesize[:60]}...', Assistant prefix starts with: '{assistant_content_prefix_str[:60]}...'")  # noqa: E501
            prompt_str_for_llm = chat_template_hf_tokenizer.apply_chat_template(chat_for_template, tokenize=False,
        ...
        max_new_tokens_for_llm = max(MIN_SPEECH_TOKENS_THRESHOLD, min(llm_max_context - num_prompt_tokens - 50,
            desired_max_speech_tokens_llm_can_generate))

        if max_new_tokens_for_llm <= 0:
            logger.error(f"{log_prefix}Calculated max_new_tokens for LLM is {max_new_tokens_for_llm}, which is too low (prompt too long for context). Prompt tokens: {num_prompt_tokens}, Max context: {llm_max_context}. Skipping LLM generation.")  # noqa: E501
            return
        ...
        if model_params_override:
            try:
                ...
                logger.warning(f"{log_prefix}Could not parse --model-params for LLM sampler settings: {e_params}. Using defaults.")  # noqa: E501

        mlx_sampler = make_sampler_func(temp=sampler_temp, top_p=sampler_top_p)
        logger.info(f"{log_prefix}Starting MLX LLM generation. max_new_tokens={max_new_tokens_for_llm}. Sampler settings: temp={sampler_temp}, top_p={sampler_top_p}")  # noqa: E501
        ...
        logger.debug(f"{log_prefix}Raw LLM output string length: {len(llm_generated_output_str)}. Starts with: '{llm_generated_output_str[:100]}...', Ends with: '...{llm_generated_output_str[-100:]}'")  # noqa: E501
        ...
        if found_eos:
            ...
            logger.debug(f"{log_prefix}Truncated completion at '{found_eos}' (pos {earliest_eos_pos}). Final part length: {len(completion_part_str)}")  # noqa: E501
        else:
                ...
                for i in range(min(len(tokens) - 10, 100)):  # Check first 100 positions max
                    ...
                    if len(set(window)) < 3:  # Too few unique tokens in a window
                        ...
                        if truncate_pos != -1:
                            ...
                            logger.info(f"{log_prefix}Detected repetitive pattern at token {i}, truncated to prevent gibberish")  # noqa: E501
                            break
        ...
        if len(numeric_speech_ids_for_xcodec) < MIN_SPEECH_TOKENS_THRESHOLD:
            logger.warning(f"{log_prefix}Number of extracted speech IDs ({len(numeric_speech_ids_for_xcodec)}) is below threshold ({MIN_SPEECH_TOKENS_THRESHOLD}).")  # noqa: E501
            if actual_ref_speaker_wav_for_processing and attempt_no_cloning_fallback and not _is_fallback_attempt:
                logger.info(f"{log_prefix}Output too short with voice cloning. Attempting RETRY WITHOUT CLONING (zero-shot fallback).")  # noqa: E501
                ...
                if temp_trimmed_ref_audio_file_to_delete and temp_trimmed_ref_audio_file_to_delete.exists():
                    try:
                        ...
                        pass
                _cleanup_resources(llasa_llm_mlx_model, llasa_llm_mlx_tokenizer, chat_template_hf_tokenizer,
                    codec_model_pt_inst)
                # Recursive call for fallback
                synthesize_with_llasa_hybrid(model_config, text_to_synthesize, None, model_params_override,
                    output_file_str, play_direct, False, True)
                return
            else:
                logger.error(f"{log_prefix}Output too short, and no further fallbacks applicable (or fallback already attempted). Skipping XCodec2 decoding.")  # noqa: E501
                return

        # Decode speech IDs to waveform using XCodec2
        speech_ids_tensor_for_xcodec = torch_llasa.tensor(numeric_speech_ids_for_xcodec,
            device=pt_device_obj).unsqueeze(0).unsqueeze(0)
        ...
        with torch_llasa.no_grad(), SuppressOutput_util(suppress_stdout=suppress_external_lib_logs,
            suppress_stderr=suppress_external_lib_logs):
        ...
        if is_cloning_mode and processed_ref_audio_np is not None:
            ...
            if generated_waveform_pt.shape[2] > ref_samples:
                ...
                logger.warning(f"{log_prefix}Generated audio ({generated_waveform_pt.shape[2]} samples) shorter than reference ({ref_samples} samples). Cannot trim reference portion.")  # noqa: E501
        ...
        logger.info(f"{log_prefix}XCodec2 decoding successful. Generated audio duration: {duration_seconds:.2f}s at {target_sample_rate}Hz.")  # noqa: E501
        ...
        if output_file_str and save_audio_util:
            try:
                ...
                audio_int16_bytes = (np.clip(audio_output_np.astype(np.float32), -1.0,
                    1.0) * 32767).astype(np.int16).tobytes()
                save_audio_util(audio_int16_bytes, str(output_path_obj), source_is_path=False, input_format="pcm_s16le",
                    sample_rate=target_sample_rate)
                logger.info(f"{log_prefix}Synthesized audio successfully saved to: {output_path_obj}")
            except Exception as e_save:
                logger.error(f"{log_prefix}Failed to save synthesized audio to '{output_path_obj}': {e_save}",
                    exc_info=True)

        if play_direct and play_audio_util:
            try:
                logger.info(f"{log_prefix}Attempting to play synthesized audio directly...")
                audio_int16_bytes_for_play = (np.clip(audio_output_np.astype(np.float32), -1.0,
                    1.0) * 32767).astype(np.int16).tobytes()
                play_audio_util(audio_int16_bytes_for_play, is_path=False, input_format="pcm_s16le",
                    sample_rate=target_sample_rate)
            except Exception as e_play:
                logger.error(f"{log_prefix}Failed to play synthesized audio directly: {e_play}", exc_info=True)

        logger.info(f"{log_prefix}Synthesis process completed successfully{'.' if not _is_fallback_attempt else ' (after fallback to zero-shot).'}")  # noqa: E501

    except Exception as e_main_synth:
        logger.error(f"{log_prefix}An unexpected error occurred during the main synthesis block: {e_main_synth}",
            exc_info=True)
    finally:
        ...
        if temp_trimmed_ref_audio_file_to_delete and temp_trimmed_ref_audio_file_to_delete.exists():
            try:
                ...
                logger.debug(f"{log_prefix}Successfully deleted temporary trimmed reference audio: {temp_trimmed_ref_audio_file_to_delete}")  # noqa: E501
            except OSError as e_del_temp:
                logger.warning(f"{log_prefix}Could not delete temporary trimmed reference audio '{temp_trimmed_ref_audio_file_to_delete}': {e_del_temp}")  # noqa: E501
...
def test_llasa_basic_functionality() -> bool:
    ...
    try:
        if not all([MLX_LM_AVAILABLE, TORCH_FOR_LLASA_AVAILABLE, XCODEC2_AVAILABLE, TRANSFORMERS_FOR_LLASA_AVAILABLE,
        ...
        test_tokens_str = "".join(_llasa_ids_to_speech_tokens_str(test_ids))
        if _llasa_extract_speech_ids_from_str_list(test_tokens_str) != test_ids:
            logger.error("LLaSA Basic Functionality Test: Speech token utils failed.")
            return False
        ...
        logger.error(f"LLaSA Handler: Basic functionality test failed: {e}", exc_info=True)
        return False
...
def _log_initialization_status_detailed():
    ...
    if is_functional:
        ...
        if not test_llasa_basic_functionality():
            logger.warning("LLaSA: Basic functionality test FAILED despite dependencies appearing met. Check detailed logs for test_llasa_basic_functionality.")  # noqa: E501
    else:
...
if __name__ != "__main__":
    ...
    if not UTILS_AVAILABLE:
        logger_init.critical("LLaSA Hybrid Handler CRITICAL: Project 'utils.py' components not imported. Key functionalities will fail.")  # noqa: E501
